...
describe UserMailer do
  ...
  let(:user)        { create(:user, password_reset_token: "random") }
  let(:billing)     { create(:address, address_type: 'billing') }
  let(:shipping)    { create(:address, address_type: 'shipping') }
  let(:transaction) { create(:transaction) }
  let(:cart)        { create(:cart) }
  let(:order)       { create(:order, cart: cart, addresses: [billing, shipping], transactions: [transaction]) }
  ...
  describe "#order_received(order)" do
    ...
    it "renders the body" do
      expect(mail.body.encoded).to include("Dear #{billing.first_name} #{billing.last_name}")
    end
  end
